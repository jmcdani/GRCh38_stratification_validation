...
###########################################################################
### create Dataframe of BEDs -- working with merged bed files (need to add
### note that merging was done in bash using mergeBEDs.sh script)
###########################################################################
#create vector of file paths to be used for dataframes
all_38_beds <- list.files("./data/stratifications/GRCh38/merged/",
                          full.names = TRUE)
all_37_beds <- list.files("./data/stratifications/GRCh37/merged/",
                          full.names = TRUE)
...
#col_types need to be specified becauuse reader tries to use the first 1000
#rows to infer the datatype. GRCh37 chromsome go from 22 -> X therefore datatype
#changes.  col_types=  = "cdd" set the data types for the columns to character,
#double, double

#note had issue with refseq_union_cds.sort.bed.gz, this file turns out was empty
#and was throwing an error, "Error in read_tokens_(data, tokenizer, col_specs,
#col_names, locale_:Cannot read file /Users/jmcdani/Documents/GiaB/Benchmarking/GRCh38_stratification_validation/data/stratifications/GRCh37/merged/foo: Invalid argument"
#Nate helped identify the problem file using safe_tsv <- safely(read_tsv) and
#transpose functions. This was only an issue with the GRCh37

#change stratification column to just the base name of file, removing path
all_38_beds_df$stratification_filename <-
  basename(all_38_beds_df$stratification_filename)
all_37_beds_df$stratification_filename <-
  basename(all_37_beds_df$stratification_filename)

###########################################################################
## Joining Dataframes using stratfile_summary
###########################################################################
#join dataframes of beds and stratification levels
stratfile_summary <- read.csv("./data/stratification_summary_spreadsheets/110619_37and38_stratification_types_levels_md5_longFormat.csv",header = TRUE, na.strings = TRUE)
...
###########################################################################
## Plotting to look at chromosome presence by reference
###########################################################################
#vector of chromosomes we expect to have
expected_chroms <- c("chr1",
                     "chr2",
                     "chr3",
                     "chr4",
                     "chr5",
...
distinct(expected_chroms_38 %>%
           select(chrom, stratification_type, stratification_level)) %>%
  ggplot(aes(x=as.factor(stratification_level), y =as.factor(chrom))) +
  geom_point(aes(col=stratification_type), show.legend = FALSE) +
  labs(x="stratification", y="chrom",
       title="Expected Chromosome Prescence 38 stratifications") +
  theme(axis.text.x = element_text(angle = 90))

distinct(expected_chroms_37 %>%
           select(chrom, stratification_type, stratification_level)) %>%
  ggplot(aes(x=as.factor(stratification_level), y =as.factor(chrom))) +
  geom_point(aes(col=stratification_type), show.legend = FALSE ) +
  labs(x="stratification", y="chrom",
       title="Chromosome Prescence 37 stratifications") +
  theme(axis.text.x = element_text(angle = 90))

#plot UNexpected chroms
distinct(unexpected_chroms_38 %>%
           select(chrom, stratification_type, stratification_level)) %>%
  ggplot(aes(x=as.factor(stratification_level), y =as.factor(chrom))) +
  geom_point(aes(col=stratification_type), show.legend = FALSE) +
  labs(x="stratification", y="chrom",
...
get_chrom_sizes <- function(genome){
  ...
  #had to adjust list to accomodate 24 elements to include X and Y.  Original
  #code only used chroms 1:22
...
###########################################################################
#get chrom sizes w/o Ns from references using Nate's functions
###########################################################################
...
#Coverage Plots
functional <-filter(summary_for_plots, stratification_type == "FunctionalRegions")
ggplot(functional, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "FunctionalRegions")

GC <-filter(summary_for_plots, stratification_type == "GCcontent")
ggplot(GC, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "GCcontent")

Mappability <-filter(summary_for_plots, stratification_type == "mappability")
ggplot(Mappability, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "Mappability")

SD <-filter(summary_for_plots, stratification_type == "SegmentalDuplications")
ggplot(SD, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "SegmentalDuplications")

TechDiff <-filter(summary_for_plots,
                  stratification_type == "FunctionalTechnicallyDifficultRegions")
ggplot(TechDiff, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "FunctionalTechnicallyDifficultRegions")


LowComplex <-filter(summary_for_plots, stratification_type == "LowComplexity")
ggplot(LowComplex, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference), position = "dodge") +
  facet_grid(scales = "free_y", stratification_level ~ .) +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
  labs(title = "LowComplexity")
...
ggplot(GSpecific, aes(x=chrom, y=coverage, group = reference)) +
  geom_col(aes(fill = reference ), position = "dodge") +
  facet_grid(stratification_level ~ genome, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90), legend.position = "top",
        strip.text.y = element_text(angle = 0)) +
